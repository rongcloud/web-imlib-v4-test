(function (win, dependencies, components) {
  ...
  function watchPullFinished() {
    vueInstance.addOutput('离线数据加载完毕', null, 0, [], {
  ...
  function watchTag() {
    vueInstance.addOutput('监听到标签变化', '', 0, [], {
      color: utils.TypeColor.MSG
    });
  }
  function watchConversation (event) {
    vueInstance.addOutput('监听到会话标签变化', '', 0, [], {
      color: utils.TypeColor.MSG
    });
  }
  ...
  function watchTypingStatus(event) {
    vueInstance.addOutput('监听到typing状态改变', event, 0, [], {
      color: utils.TypeColor.MSG
    });
  }

  function watchMessageBlocked(event) {
    vueInstance.addOutput('监听到敏感消息被屏蔽', event, 0, [], {
      color: utils.TypeColor.MSG
    });
  }
  ...
  function login(config) {
    ...
    return Service.init(config, {
      ...
      typingStatus: watchTypingStatus,
    ...
    computed: {
      ...
      currentOutput: function() {
        ...
        if (outputList.length) {
          let obj=outputList[outputList.length - 1];
          console.log('obj',obj)
          return obj;
        }
      },
